...
int main(int argc,char ** argv )
{
  ...
  load_from_file_distributed_A2A ( argc, argv, &N, &N_glo, NDIM, &points, N_p, rank);
#else
  // ELSE GENERATE RANDOM DATA    
  generate_random_points_distributed_A2A ( argc, argv, &N, &N_glo, NDIM, &points, N_p, rank);
#endif
  ...
  show_array_A2A( points, 0, N, level, 'A') ;
#endif
  ...
  MPI_Barrier(MPI_COMM_WORLD);
  start = MPI_Wtime();
  build_kdtree_distributed_A2A(root, points, N, NDIM, axis, level, MPI_COMM_WORLD);
  end = MPI_Wtime();
  MPI_Barrier(MPI_COMM_WORLD);
  time = end-start;
